...
import random
import threading
from utiles_v2 import parse_dialog_history
...
tool_use_weight = [0.35,0.35,0.1,2]
tool_use_weight_loss_info = [0.35,0.35,0.1,2]
intent = ['事实查询（百科、新闻）。',
          '生活决策（穿衣、饮食、运动）。',
          '情感宣泄（抱怨、开心、焦虑）。',
          '记忆回溯（“我上次...”）。',
...
def make_prompt(result,background):
    ...
    prompt = f'''请生成一段用户和它的人工智能助手jarvis的对话,user query我会提供给你
        ...
        (备注:Thought,Tool Call,Output可能会重复多轮，取决于用户的问题，有些问题需要多轮工具调用才能解决)
        User: user query
        ...
        Assistant Final think: Final response前的思考
...
def generate_sft_data():
    ...
    result_, think_ = ds_r1.get_complete([make_user_message(prompt)])
    print(result_)
    constructed_data.append(ast.literal_eval(result))
    ...
    with open("jarvis_chat_v2.jsonl", 'a', encoding='utf-8') as f:
        f.write(json_line + '\n')
